...
local hitboxspeed=5 --hitboxspeed
local hitbox=1 --hitbox related with the difficulty of hit
local combo_num=0
...
combo.y = 100
combo.alpha= 0.7
local combo_score = display.newText(combo_num,550,130,system.nativeFont,20)
...
local act = function(event)
    ...
    if switch == 0 and event.phase == "began" then
        if event.x>-195 and event.x<-45 and switch==0 then
            switch=1
            local space = display.newImageRect( "images/crate.png", 100, hitbox)
            space.x = -120 
            space.y = 410
            space.alpha=0
            local  function del()
            ...
            transition.to(space, {time=hitboxspeed, x=space.x, y=space.y+1, rotation=0,onComplete=del})
